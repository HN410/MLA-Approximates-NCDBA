...
from utils.model_units import calc_opt_theta, calc_add_theta, calc_mult_theta
...
def calc_and_save_mean_features(
        base_args: BaseArgs,
        dataloader: DataLoader,
        model: MyModel,
        label_list: list,
        processed_data: tuple = None,
        phase: str = "train",
        save: bool = False
    ):
    assert (processed_data is not None) or (dataloader is not None and model is not None and label_list is not None), "Set dataloader, model and label_list, or processed_data"
    
    if processed_data is not None:
        ans_dict = processed_data
    else:
        ans_dict = process_all_data(dataloader, base_args, model,
                                    (FEATURES_MAT_KEY, LABEL_KEY),
                                    label_list)
    features_mat = ans_dict[FEATURES_MAT_KEY]
    new_label_tensor = ans_dict[LABEL_KEY]
    
    # クラス平均特徴量ベクトルを求める
    mean_features = torch.stack([features_mat.to(base_args.device)[ new_label_tensor == i].mean(0)
                          for i in range(base_args.data.n_classes)])
    if save:
        np.save(os.path.join(get_work_dir_path(base_args), MEAN_FEATURES_FORMAT.format(phase)), mean_features.to("cpu").numpy())
    return mean_features
    

def plot_theta_map(
    img_num_per_cls: list,
    ...
    use_best = best_tuple is not None
    tau_opt = best_tuple[0] if use_best else 0.5
    opt_theta = calc_opt_theta(img_num_per_cls, feature_norms, tau_opt)
    if additive:
        tau = best_tuple[1] if use_best else 1
        theta = calc_add_theta(img_num_per_cls, feature_norms, tau)
        res = theta.cpu() - opt_theta.cpu()
    else:
        tau = best_tuple[1] if use_best else 0.5
        theta = calc_mult_theta(img_num_per_cls, tau)
        res = theta.cpu() - opt_theta.cpu()
    
    print(torch.abs(res).mean())
    plt.figure(figsize=(10, 10), facecolor="white")
    plt.title(args.exp_name, fontsize=28)
    seaborn.heatmap(res.numpy(), square= True, annot=False, cmap="RdBu_r", cbar = True, vmin = -0.5, vmax=0.5)
    
    title = "dif_of_{}_theta{}".format("add" if additive else "mult", "_best" if use_best else "")
    if save_map:
        npy_dir = os.path.join(get_work_dir_path(args), "theta_map")
        if not os.path.exists(npy_dir):
            os.makedirs(npy_dir)
        np.save(os.path.join(npy_dir, title+".npy"), res.numpy())
    
    return output_plot(title, args, tight=True, output_image=output_image)
