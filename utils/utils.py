...
MEAN_FEATURES_FORMAT = "mean_features_{}.npy"
...
class ModelArgs(Args):
    def __init__(self, vars_dict=None):
        ...
        self.load_pretrained: bool = True
        ...
        self.onevsone_adjuster_tau: float = 0.0
        self.onevsone_new: bool = False
        self.use_feature_norms_for_onevsone: bool = False
...
class BaseArgs(RootArgs):
    ...
    def __init__(self, vars_dict= None, add_logger = True):
        ...
        self.pause_epoch: int = -1
        self.save_interval: int = -1
        self.sp_exp_name: str = ""
...
def get_work_dir_path(args: RootArgs):
    if type(args) == BaseArgs:
        if args.sp_exp_name:
            work_dir=os.path.join(os.getcwd(), "exp_sp", args.sp_exp_name)
        else:
...
def log_result_path(args: RootArgs):
    args.logger.info(get_work_dir_path(args))
