...
from utils.models_doda import get_doda_model
...
def get_base_model(args: BaseArgs, outFeature=False, train_img_num_per_class = None, get_residual = False, m_feature_norms = None):
    #TODO support outFeature for ResNext50
    fc_bias_param_name = None
    use_etf = args.model.use_etf
    
    model: MyModel
    assert not((args.model.logit_adjustment_tau > 0) and (args.model.onevsone_adjuster_tau > 0)) , "Cannot use both logit_adjustment and onevsone_adjuster"
    if args.model.logit_adjustment_tau > 0:
        ...
        logit_adjuster = OneVSOneAdjuster(args.model.onevsone_adjuster_tau, train_img_num_per_class, m_feature_norms, new = args.model.onevsone_new)
    ...
    elif args.model.name.startswith("DODAResNet"):
        assert not use_etf, "DODAResNet does not support ETF"
        assert not args.model.use_dr_balancer, "DODAResNet does not support DRBalancer"
        
        model = get_doda_model(logit_adjuster)
    else:
