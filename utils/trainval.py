...
def set_model(args: BaseArgs, train=True, outFeature=False, train_img_num_per_class = None, load_best = True, get_residual = False,
              best_param_path: str = None, feature_norms_path: str = None):
    assert not (train and get_residual), "No support for get_residual when training"
    
    # Load feature_norms for onevsone_adjuster
    load_feature_norm = (args.model.onevsone_adjuster_tau > 0) and (args.model.use_feature_norms_for_onevsone) and (args.model.is_pretrained)
    if load_feature_norm:
        feature_norms_path = os.path.join(args.model.pretrained_path, str(args.seeds), MEAN_FEATURES_FORMAT.format("train")) if feature_norms_path is None else feature_norms_path
        feature = np.load(feature_norms_path)
        feature = torch.tensor(feature).to(args.device)
        feature_norms = torch.norm(feature, dim=1)
    else:
        feature_norms = None
        
    
    base_model: MyModel = get_base_model(args, outFeature=outFeature,
                                         train_img_num_per_class=train_img_num_per_class, get_residual=get_residual, m_feature_norms=feature_norms)
    
    best_param_path = os.path.join(get_work_dir_path(args), BEST_PARAM_FILE_NAME) if best_param_path is None else best_param_path
    if os.path.exists(best_param_path) and load_best:
        ...
        if args.model.is_pretrained and args.model.load_pretrained:
...
def check_statistics_per_epoch(writer: SummaryWriter, model: MyModel, epoch: int, dataloaders_dict: dict,
    ...
    check_interval = CHECK_INTERVAL if args.data.n_classes > 100 else 5
    if epoch % check_interval == 0:
        ...
        if args.data.n_classes <=  100 or (epoch % (check_interval * 5) == 0):
...
CHECK_SCHED_KEY = "scheduler"

def save_check_point(args: BaseArgs, epoch: int, model: MyModel, optimizer: optim.Optimizer, scheduler: optim.lr_scheduler._LRScheduler):
    check_dict = {}
    check_dict[CHECK_EPOCH_KEY] = epoch
    check_dict[CHECK_MODEL_KEY] = model.state_dict()
    check_dict[CHECK_OPT_KEY] = optimizer.state_dict()
    check_dict[CHECK_SCHED_KEY] = scheduler.state_dict()
...
def exists_check_point(args: BaseArgs):
    return os.path.exists(os.path.join(get_work_dir_path(args), CHECK_POINT_FILE_NAME))

def load_check_point(args: BaseArgs, model: MyModel, optimizer: optim.Optimizer, scheduler: optim.lr_scheduler._LRScheduler):
    ...
    scheduler.load_state_dict(check_dict[CHECK_SCHED_KEY])
...
def train_model(args: BaseArgs, dataloaders, new_label_list: list, img_num_per_class: list, model: MyModel, lossFunc, 
    ...
    if args.load_checkpoint:
        if exists_check_point(args):
            start_epoch = load_check_point(args, model, optimizerW, schedulerW)
            trackRecords = torch.load(os.path.join(work_dir, TRACK_RECORDS_FILE_NAME))
            
            trackRecords[TF_RECORDS_KEY] = list(trackRecords[TF_RECORDS_KEY])
            if FEATURES_NORM_KEY in trackRecords:
                trackRecords[FEATURES_NORM_KEY] = list(trackRecords[FEATURES_NORM_KEY])
    

            
            logger.info("Checkpoint loaded. Starting from epoch {}".format(start_epoch))
        else:
            logger.info("No checkpoint found. Starting from scratch.")
            start_epoch = 0
        
        if start_epoch >= args.optim.total_epoch_num:
            logger.info("Training already finished.")
            return trackRecords
    else:
        start_epoch = 0
    confMat = None
    for epoch in range(start_epoch, num_epochs):  
        ...
        for phase in phaseList:
            ...
            log_fdr = LOG_FDR and (args.data.n_classes <= 100)
            ...
            if (phase==VALID_DATA_KEY or phase==TEST_DATA_KEY) and curPerClassAcc>best_perClassAcc: #epoch_loss<best_loss:            
                ...
                if save:
                    ...
                    fn.close()
            if phase==TRAIN_DATA_KEY and args.save_interval > 0 and (((epoch+1) % args.save_interval == 0) or (epoch == num_epochs - 1)):
                saves_dir = os.path.join(work_dir, "ckpts")
                if not os.path.exists(saves_dir):
                    os.makedirs(saves_dir)
                path_to_save_param = os.path.join(saves_dir, "epoch_{:04}.ckpt".format(epoch))
                torch.save(model.state_dict(), path_to_save_param)
                mean_features = calc_and_save_mean_features(args, None, None,None,  ans_dict, "train")
                np.save(os.path.join(saves_dir, "m_features_{:04}.npy".format(epoch)), mean_features.to("cpu").numpy())
                
                
            
        check_statistics_per_epoch(writer, model, epoch, dataloaders, args, new_label_list, img_num_per_class, lossFunc)
        if args.save_checkpoint:
            save_check_point(args, epoch, model, optimizerW, schedulerW)
        if args.pause_epoch > 0 and ((epoch+1) % args.pause_epoch) == 0:
            print("Paused at epoch {}".format(epoch))
            break
...
def check_model_statistic(args: BaseArgs, models_dict: dict, dataloaders_dict: dict,
    ...
    if track_records and args.optim.total_epoch_num > 0:
    ...
    with OutFeatureChanger(model, True):
        ...
        calc_and_save_mean_features(args, None, None, None, ans_dict_train, save = True)
        ...
        calc_and_save_mean_features(args, None, None, None, ans_dict_test, phase="test", save = True)
