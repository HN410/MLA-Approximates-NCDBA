...
def calc_opt_theta(img_num_per_cls: list = None, feature_norms: torch.Tensor = None, tau: float = 0.5):
    rho = np.arccos(-1/(len(img_num_per_cls)-1))
    img_num_per_cls = torch.tensor(img_num_per_cls, dtype = torch.float32)
    img_num_per_cls = torch.pow(img_num_per_cls, tau)
    
    if feature_norms is not None:
        adjusters = img_num_per_cls * feature_norms.to("cpu")
    else:
        adjusters = img_num_per_cls
    # theta_cc' ... [c, c']
    theta_cc = adjusters.unsqueeze(0) + adjusters.unsqueeze(1)
    theta_cc = adjusters.unsqueeze(0) / theta_cc
    theta_cc = rho * theta_cc
    return theta_cc

def calc_mult_theta(img_num_per_cls: list = None, tau: float=0.5):
    rho = np.arccos(-1/(len(img_num_per_cls)-1)).item()
    img_num_per_cls = torch.tensor(img_num_per_cls, dtype = torch.float32)
    img_num_per_cls = torch.pow(img_num_per_cls, tau)
    
    theta_cc = img_num_per_cls.unsqueeze(0) / img_num_per_cls.unsqueeze(1)
    theta_cc += np.sin(np.pi / 2 - rho).item()
    theta_cc /= np.cos(np.pi / 2 - rho).item()
    return torch.arctan(theta_cc)

def calc_add_theta(img_num_per_cls: list = None, feature_norms: torch.Tensor = None, tau: float=1): 
    rho =np.arccos(-1/(len(img_num_per_cls)-1))
    img_num_per_cls = torch.tensor(img_num_per_cls, dtype = torch.float32).to(feature_norms.device)
    
    # calc weighted mean of norms 
    mean_norms = torch.sum(img_num_per_cls * feature_norms) / torch.sum(img_num_per_cls)
    theta_cc = tau * torch.log(img_num_per_cls.unsqueeze(1) / img_num_per_cls.unsqueeze(0))
    theta_cc = theta_cc / (2 * mean_norms * np.sin(rho / 2))
    theta_cc = rho / 2 - torch.arcsin(theta_cc)
    return theta_cc    
    
    
class OneVSOneAdjuster(nn.Module):
    def __init__(self, tau: float= 0.5, img_num_per_cls: list = None, feature_norms: torch.Tensor = None, new: bool = True):
        super().__init__()
        self.tau = tau
        if tau > 0:
            assert img_num_per_cls is not None, "img_num_per_cls should not be None when tau > 0"
            theta_cc = calc_opt_theta(img_num_per_cls, feature_norms, tau)
            if  torch.sum(theta_cc >= np.pi/2) > 0:
                print("Distribution is too skewed or class number is too small.")
                print("OneVSOne adjuster may not work well.")
            if new:
                cos_rho = -1/(len(img_num_per_cls)-1)
                sin_cc = torch.sin(theta_cc).unsqueeze(0)
                trans_sin_cc = (torch.transpose(sin_cc, 2, 1))
                kappa_cc = sin_cc + cos_rho * trans_sin_cc
            else:
                kappa_cc = torch.cos(theta_cc).unsqueeze(0)
                kappa_cc = 1/kappa_cc
            
            
            
            self.register_buffer("kappa_cc", kappa_cc, persistent=False)
            
    
    def forward(self, x):
        # x: [B, C]
        if self.tau > 0:
            pos_x = x.unsqueeze(2) * self.kappa_cc
            neg_x = x.unsqueeze(1) * (torch.transpose(self.kappa_cc, 2, 1))
            class_count = (((pos_x - neg_x) > 0)).sum(dim = 2).to(torch.float32) / x.shape[1]
            return class_count
        return x
